...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-space-dark text-slate-100 flex flex-col font-sans relative overflow-hidden">
      ...
      {/* Deep Nebula Glow Overlays */}
      <div className="absolute top-1/4 left-1/4 w-[500px] h-[500px] rounded-full nebula-glow-1 -z-10 pointer-events-none"></div>
      <div className="absolute top-2/3 right-1/4 w-[600px] h-[600px] rounded-full nebula-glow-2 -z-10 pointer-events-none"></div>
      <div className="absolute bottom-10 left-10 w-[400px] h-[400px] rounded-full nebula-glow-3 -z-10 pointer-events-none"></div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-black text-white tracking-tight leading-none">
                PianetaSim <span className="text-fluo-green text-sm font-bold bg-emerald-950/40 px-2 py-0.5 rounded-md ml-1 border border-fluo-green/30 shadow-[0_0_10px_rgba(38,255,94,0.15)]">GUIDE</span>
              </h1>
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="max-w-3xl space-y-4">
            ...
            <h2 className="text-3xl md:text-5xl font-black text-white tracking-tight leading-none md:leading-tight">
              Tutto quello che devi sapere su <span className="text-fluo-green drop-shadow-[0_0_10px_rgba(38,255,94,0.3)]">The Sims 4</span>
            </h2>
          ...
          {activeTab === 'guides' && (
            ...
            >
              ...
              <div className="lg:col-span-3 space-y-6">
                ...
                <div className="bg-space-card p-5 rounded-2xl border border-slate-800 shadow-xl space-y-4 neon-shadow-blue">
                  <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                    Seleziona Categoria
                  </h4>
                  <div className="space-y-1.5">
                    {[
                      ...
                      <button
                        ...
                        className={`w-full text-left px-3 py-2 rounded-xl text-xs font-semibold flex items-center justify-between transition-all cursor-pointer ${
                          selectedCategory === cat.id && !showBookmarksOnly
                            ? 'bg-slate-800 text-fluo-blue shadow-inner'
                      ...
                      >
                        ...
                        <span className={`text-[10px] px-2 py-0.5 rounded-full ${
                          selectedCategory === cat.id && !showBookmarksOnly
                            ? 'bg-fluo-blue text-black font-extrabold shadow-[0_0_8px_rgba(0,240,255,0.4)]'
                    ...
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 rounded-xl text-xs font-semibold flex items-center justify-between transition-all mt-4 border cursor-pointer ${
                        showBookmarksOnly
                          ? 'bg-amber-500/10 text-amber-400 border-amber-500/30 font-bold shadow-[0_0_8px_rgba(245,158,11,0.2)]'
                          : 'text-slate-400 hover:text-white hover:bg-slate-800/40 border-transparent'
                      }`}
                    >
                      <span className="flex items-center gap-1.5">
                        <Star className={`w-3.5 h-3.5 ${showBookmarksOnly ? 'fill-current text-amber-400' : ''}`} />
                        I Miei Preferiti
                      </span>
                      <span className={`text-[10px] px-2 py-0.5 rounded-full ${
                        showBookmarksOnly ? 'bg-amber-500 text-white' : 'bg-slate-900 text-slate-500'
                      }`}>
                        {bookmarks.length}
                      </span>
                    </button>
                ...
                <div className="bg-space-card p-5 rounded-2xl border border-slate-800 shadow-xl space-y-3 neon-shadow-fuchsia">
                  <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                      ...
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer border ${
                          selectedDifficulty === diff
                            ? 'bg-fluo-fuchsia text-white border-fluo-fuchsia shadow-[0_0_8px_rgba(255,0,127,0.4)]'
      ...
      {/* 4. Footer section */}
      <footer className="bg-slate-950 border-t border-slate-900 text-slate-400 py-12 px-4 md:px-8 mt-12 text-xs relative z-10">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="space-y-3">
            <h4 className="text-sm font-bold text-white flex items-center gap-1.5">
              <span className="w-2.5 h-2.5 rounded-full bg-fluo-green shadow-[0_0_8px_#26ff5e]" />
